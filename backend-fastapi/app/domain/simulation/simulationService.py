...
def _build_features(
    ...
    for col in _industry_columns:
        row[col] = (
            1.0 if _nfc(_industry_map.get(col, "").replace("업종_", "")) == nfc_industry else 0.0
        )
...
async def predict_survival(db: AsyncSession, request: PredictionRequest) -> PredictionResponse:
    ...
    if request.industry not in _industry_list:
        raise HTTPException(
            status_code=400, detail=f"지원하지 않는 업종입니다. 가능 업종: {_industry_list}"
        )
    ...
    if not ab:
        raise HTTPException(
            status_code=404, detail=f"행정동을 찾을 수 없습니다. (adm_cd={request.adm_cd})"
        )
    ...
    if pred:
        factors.append(
            ...
            if dong_freq > 0.01
            else f"{ab.adm_nm} 지역의 유동인구 대비 해당 업종의 결제 전환율이 다소 낮게 예측됩니다."
        )
        ...
        factors.append(
            ...
            if dong_freq < 0.01
            else f"{ab.adm_nm} 지역의 해당 업종 소비 활성도가 안정적인 구간에 진입해 있습니다."
        )
