...
def final_score(similarity: float, survival_rate_3y: float | None) -> float:
    survival = (survival_rate_3y / 100) if survival_rate_3y else 0.5  # None이면 중간값
    return round(similarity * 0.8 + survival * 0.2, 4)

async def get_equipment_by_ids(db: AsyncSession, equipment_ids: list[UUID]) -> list[dict]:
    result = await db.execute(text("""
                                   SELECT id, name, category
                                   FROM equipment
                                   WHERE id = ANY(:ids)
                                   """), {"ids": [str(i) for i in equipment_ids]})
...
async def recommend_industry_change(
    ...
    eq_index = {str(e["id"]): i for i, e in enumerate(all_eq)}  # id → 벡터 인덱스
    n_eq = len(all_eq)
