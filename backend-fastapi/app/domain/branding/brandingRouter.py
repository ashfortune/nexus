...
from app.domain.branding import brandingSchema, brandingService
...
async def start_branding(
    ...
    try:
        ...
        return brandingSchema.BrandingResponse(
            success=True, data=brandingSchema.BrandingResponseData(projectId=new_project.id)
        )
...
async def start_chat(
    branding_id: uuid.UUID, request: brandingSchema.ChatRequest, db: AsyncSession = Depends(get_db)
):
    """
    AI 인터뷰 채팅 API
...
async def generate_brand_names_api(branding_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """
    브랜드 명 생성 API
    ...
    try:
        ...
        if not results_data:
            raise HTTPException(
                status_code=400, detail="인터뷰 데이터가 부족하거나 프로젝트를 찾을 수 없습니다."
            )
        ...
        return brandingSchema.NamingResponse(success=True, data=results)
    except HTTPException as he:
        raise he
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"브랜드 명 생성 중 오류가 발생했습니다: {str(e)}"
        )


@router.post("/identity/{identity_id}/logo", response_model=brandingSchema.LogoResponse)
async def generate_logo_api(identity_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """
    브랜드 로고 생성 API
    ...
    try:
        ...
        if not candidates:
            raise HTTPException(
                status_code=404, detail="해당 브랜드 아이덴티티를 찾을 수 없습니다."
            )

        return brandingSchema.LogoResponse(
            success=True, data=[brandingSchema.LogoCandidate(**c) for c in candidates]
        )
    except Exception as e:
...
@router.post(
    "/identity/{identity_id}/logo/finalize", response_model=brandingSchema.LogoFinalizeResponse
)
...
async def generate_marketing_assets_api(
    ...
    try:
        ...
        if not assets:
            raise HTTPException(
                status_code=404, detail="에셋을 생성할 수 없습니다. 로고 확정 여부를 확인해주세요."
            )

        return brandingSchema.BrandingAssetsResponse(
            success=True, data=[brandingSchema.BrandingAssetResult(**a) for a in assets]
        )
    except Exception as e:
