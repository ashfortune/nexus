...
public interface BoardRepository extends JpaRepository<Board, UUID> {
        ...
        @Query("SELECT b FROM Board b WHERE b.categoryName = 'FREE' ORDER BY b.viewCount DESC")
        java.util.List<Board> findFreeBoardTop3(Pageable pageable);

        @Query("SELECT b FROM Board b WHERE b.categoryName = 'FREE' AND b.likeCount >= :likeCount")
        Page<Board> findFreeBoardPopular(@Param("likeCount") int likeCount, Pageable pageable);
        ...
        @Query("SELECT b FROM Board b WHERE b.categoryName = 'REGION' AND b.regionName = :regionName AND (" +
                        ...
                        "LOWER(b.content) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "(b.isAnonymous = false AND LOWER(b.user.nickname) LIKE LOWER(CONCAT('%', :keyword, '%'))))")
        Page<Board> findByRegionNameAndKeywordAll(@Param("regionName") String regionName,
        ...
        Page<Board> findByRegionNameAndTitleContainingOrderByCreatedAtDesc(@Param("regionName") String regionName,
                        @Param("title") String title, Pageable pageable);

        @Query("SELECT b FROM Board b WHERE b.categoryName = 'REGION' AND b.regionName = :regionName AND " +
                        "b.isAnonymous = false AND LOWER(b.user.nickname) LIKE LOWER(CONCAT('%', :nickname, '%'))")
        Page<Board> findByRegionNameAndPublicUserNickname(@Param("regionName") String regionName,
        ...
        @Query("SELECT b FROM Board b WHERE b.categoryName = 'INDUSTRY' AND b.industryCategory.id = :categoryId AND (" +
                        ...
                        "LOWER(b.content) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
                        "(b.isAnonymous = false AND LOWER(b.user.nickname) LIKE LOWER(CONCAT('%', :keyword, '%'))))")
        Page<Board> findByIndustryCategoryIdAndKeywordAll(@Param("categoryId") UUID categoryId,
                        @Param("keyword") String keyword, Pageable pageable);
}
