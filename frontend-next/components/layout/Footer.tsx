...
function Modal({ title, items, onClose }: { title: string; items: Item[]; onClose: () => void }) {
  return (
    <div
    ...
    >
      ...
      >
        <div className="flex justify-between items-center px-6 py-4 border-b border-[var(--nexus-outline-variant)]">
          <h2 className="text-xl font-black text-[var(--nexus-primary)]">{title}</h2>
        ...
        <div className="max-h-[60vh] overflow-y-auto px-6 py-5">
          <div className="grid gap-3 md:grid-cols-2">
            {items.map((item) => (
              <div key={item.id} className="rounded-xl bg-[var(--nexus-surface-container)] p-4">
                <p className="font-bold text-sm text-[var(--nexus-primary)]">
                  {item.id}. {item.title}
                </p>
                <p className="mt-2 text-sm text-[var(--nexus-on-bg)]">{item.detail}</p>
              </div>
            ))}
          </div>
        </div>
...
export default function Footer() {
  ...
  return (
    <>
      <footer className="w-full bg-[var(--nexus-surface-low)] text-[var(--nexus-on-bg)]">
        <div className="mx-auto max-w-6xl px-6 py-14">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <div className="flex flex-col gap-4">
              <p className="text-lg font-black text-[var(--nexus-primary)]">Policy</p>
            ...
            <div className="flex flex-col gap-4">
              <p className="text-lg font-black text-[var(--nexus-primary)]">Developers</p>
      ...
      {isPrivacyOpen && (
        <Modal title="개인정보처리방침" items={PRIVACIES} onClose={() => setIsPrivacyOpen(false)} />
      )}
      {isTermsOpen && (
        <Modal title="서비스 이용약관" items={POLICIES} onClose={() => setIsTermsOpen(false)} />
      )}
    </>
  );
}