...
export default function GroupBuyDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        '정말로 이 공동구매를 취소하시겠습니까? 참여자들의 결제 내역이 모두 환불 처리됩니다.'
      )
    ) {
      return;
    }
  ...
  if (isLoading)
    return (
      <div className="min-h-screen bg-[var(--nexus-bg)] flex justify-center items-center font-black text-[var(--nexus-primary)]">
        로딩 중...
      </div>
    );
  if (!gb)
    return (
      <div className="min-h-screen bg-[var(--nexus-bg)] flex justify-center items-center font-bold">
        항목을 찾을 수 없습니다.
      </div>
    );
          ...
          <div className="lg:w-3/5 space-y-12">
            <div className="relative rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white">
              ...
              <div className="absolute top-8 left-8 flex gap-3">
                <span
                  className={`px-6 py-2.5 rounded-full text-[10px] font-black shadow-2xl tracking-widest ${timeLeft.isExpired ? 'bg-slate-900 text-white' : 'bg-[var(--nexus-primary)] text-white'}`}
                >
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                <span className="text-[var(--nexus-primary)] font-black tracking-[0.3em] text-[10px] uppercase opacity-60">
                  Story & Details
                </span>
          ...
          <div className="lg:w-2/5">
            <div className="sticky top-12 nexus-card p-12 border-2 border-white shadow-2xl space-y-12">
              <div className="space-y-4">
                <span className="text-[var(--nexus-primary)] font-black text-[10px] tracking-[0.4em] uppercase opacity-50 block">
                  Nexus Collection
                </span>
                <h1 className="text-4xl font-black text-[var(--nexus-primary)] leading-tight">
                  {gb.title}
                </h1>
              ...
              <div className="space-y-1">
                ...
                <p className="text-6xl font-black text-[var(--nexus-primary)] tracking-tighter">
                  {gb.itemPrice.toLocaleString()}
                  <span className="text-2xl ml-1 font-bold opacity-30">원</span>
                </p>
              </div>

              <div
                className={`rounded-[2.5rem] p-10 border text-center transition-all ${timeLeft.isExpired ? 'bg-slate-50 border-slate-100' : 'bg-[var(--nexus-bg)] border-white shadow-inner'}`}
              >
                ...
                <div className="grid grid-cols-4 gap-6">
                  {[
                    ...
                    <div key={idx} className="flex flex-col gap-2">
                      <span
                        className={`text-3xl font-black tabular-nums ${timeLeft.isExpired ? 'text-slate-300' : 'text-[var(--nexus-primary)]'}`}
                      >
                        {String(item.value).padStart(2, '0')}
                      </span>
                      <span className="text-[10px] font-black text-slate-400 opacity-60 uppercase">
                        {item.label}
                      </span>
                    </div>
                  ))}
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-[var(--nexus-primary)] uppercase tracking-widest">
                      {progress.toFixed(0)}% 달성 완료
                    </p>
