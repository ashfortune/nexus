    ...
    const calcM = (lat: number) => {
      ...
      return (
        ...
        ((1 - e2_ / 4 - (3 * e2_ * e2_) / 64 - (5 * e2_ * e2_ * e2_) / 256) * lat -
          ((3 * e2_) / 8 + (3 * e2_ * e2_) / 32 + (45 * e2_ * e2_ * e2_) / 1024) *
          Math.sin(2 * lat) +
...
export default function StoreMapClient({ kakaoApiKey, initialIndustries, initialRegions }: any) {
  ...
  const avg = useMemo(() => {
    if (
      !storesData ||
      !storesData.storeByRegionDtoList ||
      storesData.storeByRegionDtoList.length === 0
    )
      return 0;
  ...
  const memoizedPolygons = useMemo(() => {
    ...
    storesData.storeByRegionDtoList.forEach((region: any) => {
      ...
      rings.forEach((ring: any[], pIdx: number) => {
        ...
        const path = ring.map(convertToWgs84).filter((p: any) => p !== null) as {
          lat: number;
          lng: number;
        }[];
        if (path.length >= 3) {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      if (data) {
        setStoresData(data);
        // 폴리곤 좌표에서 중심 계산 후 레벨 6으로 딱 한 번만 지도 이동
        const regions = data.storeByRegionDtoList ?? [];
  ...
  if (!isMounted)
    return (
      <div className="h-full bg-white flex items-center justify-center font-black">
        초기화 중...
      </div>
    );
  ...
  return (
    <div
      className="h-full w-full overflow-y-auto overflow-x-hidden bg-white font-inter"
      suppressHydrationWarning
    >
      <div className="flex-none h-[80vh] flex flex-col border-b border-slate-200">
        <header className="shrink-0 bg-white border-b border-slate-100 p-4 md:px-8 md:h-[72px] flex flex-col md:flex-row items-center justify-between gap-4 md:gap-12 z-40">
          ...
          <div className="flex-1 flex flex-col sm:flex-row w-full md:w-auto items-center gap-2 max-w-2xl md:px-12">
            ...
            <ChevronRight
              size={14}
              className="text-slate-300 hidden sm:block rotate-90 sm:rotate-0"
            />
            <div
              className={`relative flex-1 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 sm:py-2 flex items-center gap-3 ${!selectedRegion ? 'opacity-40' : ''}`}
            >
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'ArrowDown') {
                    e.preventDefault();
                    setActiveIndustryIndex((prev) =>
                      Math.min(prev + 1, filteredIndustries.length - 1)
                    );
                  } else if (e.key === 'ArrowUp') {
                    e.preventDefault();
          ...
          <button
            ...
            disabled={selectedRegion == '' || !selectedIndustry || isLoading}
            className="shrink-0 w-full md:w-auto h-12 md:h-10 px-8 rounded-xl text-sm font-black bg-[#0b1a7d] text-white hover:bg-indigo-900 disabled:bg-slate-200 disabled:text-slate-400 shadow-md transition-all"
          >
            {isLoading ? '분석 중...' : '상권 분석 시작'}
          </button>
        </header>

        <main className="flex-1 min-h-0 flex flex-col md:flex-row gap-4 md:gap-6 p-4 md:p-6 bg-slate-50 overflow-hidden items-stretch">
          <section className="flex-1 rounded-[24px] overflow-hidden border border-slate-200 shadow-xl relative bg-white min-h-[400px] md:min-h-0 md:h-full">
                ...
                {memoizedPolygons.map((p: any) => (
                  <React.Fragment key={p.id}>
                    <Polygon
                      ...
                      strokeColor="#0f172a"
                      strokeOpacity={0.8}
                      fillColor={p.isHigh ? '#dc2626' : '#2563eb'}
                      fillOpacity={hoveredRegion === p.region_code ? 0.7 : 0.4}
                      onMouseover={() => setHoveredRegion(p.region_code)}
                      onMouseout={() => setHoveredRegion(null)}
                    />
                    {p.isFirst && p.center && (
                      <CustomOverlayMap position={p.center} zIndex={100}>
                        <div
                          className="px-2.5 py-1.5 bg-white border-2 border-slate-950 rounded-lg shadow-2xl pointer-events-none transition-all flex flex-col items-center"
                          style={{
                            transform:
                              hoveredRegion === p.region_code
                                ? 'scale(1.1) translateY(-10px)'
                                : 'scale(1)',
                          }}
                        >
                          <span className="text-[10px] font-black text-slate-950 leading-tight whitespace-nowrap">
                            {p.region_name}
                          </span>
                          <span
                            className={`text-[9px] font-black ${p.isHigh ? 'text-red-700' : 'text-blue-700'} mt-0.5`}
                          >
                            {p.count}개
                          </span>
                        </div>
            ...
            {!sdkReady && (
              <div className="w-full h-full flex items-center justify-center bg-slate-50 text-slate-400 font-black">
                로드 중...
              </div>
            )}
          </section>
          ...
          <aside className="w-full md:w-[340px] shrink-0 flex flex-col gap-4 h-auto md:h-full scrollbar-hide overflow-y-auto">
            {storesData ? (
              <div className="bg-white rounded-[24px] border border-slate-200 p-6 shadow-lg space-y-5 h-full">
                <div className="flex items-center gap-2 text-slate-950 font-black text-[11px] uppercase tracking-wider">
                  <BarChart3 size={16} /> 분석 데이터
                </div>

                <div className="bg-indigo-50/50 rounded-2xl p-4 border border-indigo-100 flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-indigo-400 uppercase flex items-center gap-1">
                      <MapPin size={10} /> 지역
                    </span>
                    <span className="text-[12px] font-black text-indigo-950 text-right">
                      {regionSearch}
                    </span>
                  </div>
                  <div className="w-full h-px bg-indigo-100/50" />
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black text-indigo-400 uppercase flex items-center gap-1">
                      <Search size={10} /> 업종
                    </span>
                ...
                {storesData.totalCount === 0 ? (
                  <div className="py-8 px-4 bg-indigo-50 rounded-[20px] border border-indigo-100 text-center space-y-4 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <div className="space-y-2">
                      <h4 className="text-lg font-black text-indigo-950 leading-tight">
                        축하합니다!
                        <br />
                        완벽한 블루오션입니다
                      </h4>
                  ...
                  <>
                    <div className="space-y-4">
                      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex justify-between items-end">
                        <div>
                          <p className="text-[12px] font-black text-slate-500 mb-1 uppercase tracking-tight">
                            총 업소
                          </p>
                          <p className="text-3xl font-black text-slate-950 tracking-tight">
                            {storesData.totalCount.toLocaleString()}
                          </p>
                        </div>
                      </div>
                      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex justify-between items-end">
                    ...
                    <div className="pt-4 border-t border-slate-100 space-y-3">
                      {storesData.mostRegion && (
                        <div className="flex items-center justify-between p-3 bg-red-50 rounded-xl border border-red-100">
                          ...
                          <span className="text-sm font-black text-slate-950">
                            {storesData.mostRegion.adongNm || '상권'} ({storesData.mostRegion.count}
                            개)
                          </span>
                        </div>
                      )}
                      {storesData.leastRegion && (
                        <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-100">
                          ...
                          <span className="text-sm font-black text-slate-950">
                            {storesData.leastRegion.adongNm || '상권'} ({storesData.leastRegion.count}
                            개)
                          </span>
                        </div>
                      )}
              ...
              <div className="h-full flex flex-col items-center justify-center bg-white rounded-[24px] border border-slate-200 p-8 text-center shadow-lg">
                <div className="w-16 h-16 bg-indigo-50 rounded-full flex items-center justify-center mb-6 shadow-inner animate-pulse">
