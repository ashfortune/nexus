...
const ChatComponent = () => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    chatServiceRef.current.sendMessage(activeRoomId, currentUserId, inputValue, 'TALK');
    setInputValue('');
  };
  ...
  const uploadRoomImage = async (file: File): Promise<string | null> => {
    ...
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_API_URL + '/api/v1/upload/chat',
        {
          method: 'POST',
          body: formData,
        }
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-[var(--nexus-bg)] overflow-hidden font-inter text-[var(--nexus-on-bg)]">
      {/* 1. 사이드바 - 채팅방 목록 */}
      <div
        className={`${isSidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 border-r border-[var(--nexus-surface-container)] flex flex-col bg-[var(--nexus-surface-low)]`}
      >
        <div className="p-6 pb-4">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-black tracking-tighter text-[var(--nexus-primary)]">
              Messages
            </h1>
            <button
          ...
          <div className="relative group">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--nexus-outline)] group-focus-within:text-[var(--nexus-primary)] transition-colors"
              size={16}
            />
            <input
              type="text"
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <span className="font-bold text-sm truncate text-[var(--nexus-on-bg)]">
                        {room.title}
                      </span>
                    ...
                    {showAllRooms && !rooms.some((r) => r.id === room.id) && (
                      <span className="text-[9px] bg-[var(--nexus-secondary)]/10 text-[var(--nexus-secondary)] px-1.5 py-0.5 rounded-md font-bold whitespace-nowrap uppercase">
                        JOIN
                      </span>
                    )}
                  </div>
      ...
      <div className="flex-1 flex flex-col bg-white">
        {activeRoom ? (
          <>
            ...
            <div className="h-20 px-6 border-b border-[var(--nexus-surface-container)] flex items-center justify-between bg-[var(--nexus-surface-lowest)]/80 backdrop-blur-md sticky top-0 z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="font-black text-base tracking-tight text-[var(--nexus-on-bg)]">
                    {activeRoom.title}
                  </h2>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${isConnected ? 'bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-[var(--nexus-outline)]'}`}
                    ></div>
              ...
              <div className="flex items-center gap-2">
                ...
                {isMsgSearchOpen && (
                  <div className="relative animate-in slide-in-from-right-4 fade-in duration-200">
                    <Search
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--nexus-outline)]"
                      size={14}
                    />
                    <input
                      autoFocus
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {showNickname && (
                          <span
                            className={`text-[11px] font-black text-zinc-900 mb-1.5 ${isMe ? 'mr-1' : 'ml-1'}`}
                          >
                            {msg.senderNickname}
                          </span>
                        )}
                        ...
                        >
                          {msg.type === 'IMAGE' && msg.fileUrl ? (
                            ...
                            <div className="px-4 py-3">{msg.message}</div>
                          )}
                        </div>
                        <span
                          className={`mt-1.5 text-[9px] font-bold text-[var(--nexus-outline)] opacity-0 group-hover:opacity-100 transition-opacity`}
                        >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-[var(--nexus-outline)] p-12 bg-[var(--nexus-bg)]">
            ...
            <h3 className="text-xl font-black tracking-tight text-[var(--nexus-on-bg)] mb-2">
              대화를 시작해보세요
            </h3>
            <p className="text-sm font-medium text-[var(--nexus-outline)] max-w-xs text-center leading-relaxed">
              사이드바에서 채팅방을 선택하거나 '+' 버튼을 눌러 새로운 그룹 대화를 시작할 수
              있습니다.
            </p>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[var(--nexus-surface-lowest)] w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden border border-[var(--nexus-surface-container)] animate-in fade-in zoom-in duration-300">
            <div className="p-8 pb-4 flex items-center justify-between">
              <h2 className="text-2xl font-black tracking-tighter text-[var(--nexus-primary)]">
                새 채팅방 개설
              </h2>
              <button
                onClick={closeCreateModal}
            ...
            <form onSubmit={handleCreateRoom} className="p-8 pt-4 space-y-8">
              <div className="space-y-3">
                <label className="text-xs font-black text-[var(--nexus-outline)] uppercase tracking-widest ml-1">
                  방 이름
                </label>
                <input
                  autoFocus
                ...
                <div className="flex gap-4">
                  ...
                  >
                    <Users
                      ...
                      className={
                        newRoomType === 'GROUP'
                          ? 'text-[var(--nexus-on-primary)]'
                          : 'text-[var(--nexus-outline)]'
                      }
                    />
                    <div className="mt-4 font-black text-sm">그룹 채팅</div>
                    <div
                      className={`text-[10px] mt-1 font-bold ${newRoomType === 'GROUP' ? 'text-[var(--nexus-on-primary)]/70' : 'text-[var(--nexus-outline)]'}`}
                    >
                      여러 명과 대화
                    </div>
                  </button>
                  ...
                  >
                    <Plus
                      size={24}
                      className={
                        newRoomType === 'PRIVATE' ? 'text-white' : 'text-[var(--nexus-outline)]'
                      }
                    />
                    <div className="mt-4 font-black text-sm">1:1 채팅</div>
                    <div
                      className={`text-[10px] mt-1 font-bold ${newRoomType === 'PRIVATE' ? 'text-white/70' : 'text-[var(--nexus-outline)]'}`}
                    >
                      비공개 대화
                    </div>
                  </button>
                ...
                <div className="flex items-center gap-6 p-6 bg-[var(--nexus-surface-low)] rounded-2xl border-2 border-dashed border-[var(--nexus-surface-container)] hover:border-[var(--nexus-primary)]/30 transition-all group">
                  <div className="w-20 h-20 rounded-2xl bg-[var(--nexus-surface-lowest)] shadow-sm flex items-center justify-center overflow-hidden border border-[var(--nexus-surface-container)] shrink-0">
                    {imagePreview ? (
                      ...
                      <Plus
                        size={24}
                        className="text-[var(--nexus-outline)] group-hover:scale-110 transition-transform"
                      />
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="text-xs font-bold text-[var(--nexus-outline)] mb-2">
                      이미지를 선택해주세요
                    </p>
                    <input
                      type="file"
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-[var(--nexus-on-bg)]/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[var(--nexus-surface-lowest)] rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-[var(--nexus-surface-container)]">
            <div className="p-8 text-center">
              ...
              <h3 className="text-xl font-black tracking-tight text-[var(--nexus-primary)] mb-2">
                {joiningRoom.title}
              </h3>
              ...
              <div className="flex items-center justify-center gap-4 mb-8">
                <div className="flex flex-col items-center">
                  <span className="text-[10px] uppercase tracking-wider text-[var(--nexus-outline)] font-bold mb-1">
                    현재 인원
                  </span>
                  <span className="text-sm font-black text-[var(--nexus-on-bg)]">
                    {joiningRoom.participantCount || 0}명
                  </span>
                </div>
          ...
          <div className="bg-[var(--nexus-surface-lowest)] rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-[var(--nexus-error)]/20">
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-[var(--nexus-outline)] mb-8 px-4 leading-relaxed font-medium">
                정말 이 채팅방을 떠나시겠습니까?
                <br />
                <span className="font-bold text-[var(--nexus-error)]">
                  퇴장 후에는 기존 대화 내역을 볼 수 없으며, 목록에서 삭제됩니다.
                </span>
              </p>
          ...
          <div className="bg-[var(--nexus-surface-lowest)] rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-[var(--nexus-surface-container)]">
            ...
            <div className="p-4">
              <div className="relative mb-4">
                <Search
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--nexus-outline)]"
                  size={16}
                />
                <input
                  type="text"
              ...
              <div className="max-h-[350px] overflow-y-auto px-2 custom-scrollbar">
                {inviteCandidates.length > 0 ? (
                  inviteCandidates.map((user) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div
                          className={`text-[10px] font-medium ${selectedUserIds.includes(user.id) ? 'text-[var(--nexus-on-primary)]/70' : 'text-[var(--nexus-outline)]'}`}
                        >
                          {user.email}
                        </div>
                      </div>
                  ...
                  <div className="text-center py-12 text-[var(--nexus-outline)]">
                    <Users
                      size={32}
                      className="mx-auto mb-3 opacity-20 text-[var(--nexus-primary)]"
                    />
