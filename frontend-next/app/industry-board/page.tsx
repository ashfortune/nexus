...
export default function IndustryBoardPage() {
  ...
  useEffect(() => {
    if (selectedCategoryId) {
      fetchPosts(currentPage, activeTab, searchQuery, searchType, selectedCategoryId);
    }
  ...
  const handleCreatePost = () => {
    ...
    router.push(`/industry-board/create?categoryId=${selectedCategoryId}`);
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="space-y-4">
            ...
            >
              {selectedCategoryId ? `${categories.find(c => c.id === selectedCategoryId)?.name} 게시판` : '업종별 게시판'}
            </h1>
            <p className="text-zinc-500 font-medium text-lg max-w-2xl">
              {selectedCategoryId 
                ? `${categories.find(c => c.id === selectedCategoryId)?.name} 분야 사장님들과 전문적인 노하우를 공유해 보세요.` 
                : '업종별 사장님들이 모여 전문 지식과 사업 정보를 나누는 공간입니다.'}
            </p>
          </div>
        ...
        <div className="space-y-6 mb-10">
            ...
            <div className="flex gap-3">
              ...
              <div className="relative flex-1">
                <input 
                  ...
                  placeholder="전문적인 사업 지식을 검색해 보세요..."
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {categories.map((category) => (
              <button
                key={category.id}
                onClick={() => {setSelectedCategoryId(category.id); setCurrentPage(0);}}
                className={cn(
                ...
                {isLoading ? (
                  ...
                  posts.map((post, index) => (
                    <tr 
                      ...
                      onClick={() => router.push(`/industry-board/detail/${post.id}`)}
                    >
